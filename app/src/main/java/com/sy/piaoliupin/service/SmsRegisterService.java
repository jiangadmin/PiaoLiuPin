...
import com.sy.piaoliupin.activity.welcome.LoginForSMS_Activity;
...
public class SmsRegisterService {
        ...
        this.btn_register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!Util.validPhoneNumber(countryCode, phoneNumber)) {
                    TabToast.makeText("请输入有效的手机号");
                    return;
                }

                // 2. 判断验证码是否为空（是否请求了验证码由上层控制）
                if (checkCode.length() == 0) {
                    TabToast.makeText("请输入要验证码");
                    return;
                }
    ...
    class SmsRegListener implements TLSSmsRegListener {
        ...
        public void OnSmsRegCommitSuccess(TLSUserInfo userInfo) {
            TabToast.makeText("短信注册成功");
            Intent intent = new Intent(context, LoginForSMS_Activity.class);
